...
try {
  ...
  const voiceReadiness = await worker.command("Runtime.evaluate", { expression: `new Promise(resolve => chrome.tabs.query({}).then(async tabs => { const tab = tabs.find(item => typeof item.url === 'string' && item.url.includes('example.com')); resolve(await chrome.tabs.sendMessage(tab.id,{type:'voice-status',language:'zh-CN'})); }))`, awaitPromise: true, returnByValue: true });
  assert.equal(voiceReadiness.result.value?.voiceSetup, true, "The extension automatically detects that the selected Chinese voice needs setup before speaking.");
  assert.equal(voiceReadiness.result.value?.language, "zh-CN", "Automatic voice readiness keeps the exact selected locale.");
  ...
  assert.equal(chineseSpeech.result.value?.voiceSetup, true, "A missing matching voice is identified as a recoverable voice-setup condition.");
