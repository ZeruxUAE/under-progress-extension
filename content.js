...
async function readAloud(text, language, sendResponse) { if (!text) { sendResponse?.({ applied: false, error: "No readable text was found." }); return; } const selectedLanguage = typeof language === "string" ? language : ""; const voice = selectedLanguage ? await waitForVoice(selectedLanguage) : null; if (selectedLanguage && !voice) { sendResponse?.({ applied: false, language: selectedLanguage, error: `No ${selectedLanguage} voice is installed or available in this browser. Add a matching device voice, then try Read Aloud again.` }); return; } window.speechSynthesis.cancel(); const utterance = new SpeechSynthesisUtterance(text); if (selectedLanguage) { utterance.lang = selectedLanguage; utterance.voice = voice; } window.speechSynthesis.speak(utterance); sendResponse?.({ applied: true, language: selectedLanguage || undefined, voiceLanguage: voice?.lang }); }
...
if (extensionContextAvailable()) {
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    ...
    if (message.type === "speak") {
      ...
      storageGet(["underProgressProfile", "underProgressSpeechLanguage"], ({ underProgressProfile, underProgressSpeechLanguage }) => {
        ...
        readAloud(text, preferredSpeechLanguage, sendResponse);
      });
      return true;
    }
