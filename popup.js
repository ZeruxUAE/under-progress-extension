...
async function speechAction(type, success) { const tab = await activeTab(); if (!tab?.id) return setApplyStatus("Open a normal website first, then try again.", true); const result = await chrome.runtime.sendMessage({ type, tabId: tab.id }); if (!result?.applied) return setApplyStatus(result?.error || "No reading action could be completed.", true); if (type === "speak-active-tab" && result.language) return setApplyStatus(`${success} Using the saved ${result.voiceLanguage || result.language} voice.`); setApplyStatus(success); }
